...
def show_venue(venue_id):
  ...
  show_venue_artist = Show.query.filter(Show.venue == venue_id).all()
  venue = Venue.query.get(venue_id)
  past_shows = []
  upcoming_shows = []
  for show in show_venue_artist:
    if( show_venue_artist and datetime.now() > show.start_time):
      past_shows.append({
...
def show_artist(artist_id):
  # shows the venue page with the given venue_id
  # TODO: replace with real venue data from the venues table, using venue_id
  show_venue_artist = Show.query.filter(Show.artist == artist_id).all()
  artist = Artist.query.get(artist_id)
  past_shows = []
  upcoming_shows = []
  for show in show_venue_artist:
