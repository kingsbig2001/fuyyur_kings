...
def venues():
  ...
  venues = Venue.query.with_entities(Venue.name, Venue.id, Venue.city, Venue.state).all()
...
def show_venue(venue_id):
  ...
  show_venue = db.session.query(Venue, Artist, Show).filter(Venue.id == venue_id ).join(Show, Show.venue == Venue.id).join(Artist, Artist.id == Show.artist).all()
  venue = Venue.query.get(venue_id)
  past_shows = []
  upcoming_shows = []
  for show in show_venue:
    if( show_venue and datetime.now() > show.Show.start_time):
      past_shows.append({
